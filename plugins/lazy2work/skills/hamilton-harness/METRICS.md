...
Each hamilton-harness session (a chain of F1–F4 operations on one project) writes a JSON file to `hamilton_pipeline/spec_build/metrics/session-<timestamp>.json`. This lets you audit Claude's decisions later and spot drift or bypass patterns.

## File location

```
<project-root>/hamilton_pipeline/spec_build/metrics/session-2026-04-13T14-23-05Z.json
```
...
Recommended follow-up: run `/sc:analyze hamilton_pipeline/spec_build/metrics/` periodically to spot trends — e.g., if `total_retries` is consistently high on specific specs, that YAML likely has structural issues worth revisiting.
